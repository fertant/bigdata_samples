...
object HotelsStayCountHDFS {
  ...
  /** 
   *  Combine hotels data and expedia reservs with reservation type mapping.
   *
   ...
   */
  def hotelReservationTypes(spark: SparkSession, expedia: DataFrame, hotels: DataFrame): DataFrame = {
    import spark.implicits._
    expedia
      .withColumn("year", expr("year(srch_ci)"))
      .filter("year == 2017")
  ...
  /** 
   *  Count stay types by hotel.
   *
   *  @param spark spark session.
   *  @param hotelsReservation hotels reservation DataFrame.
   *  @return DataFrame with reservation mapped with types.
   */
  def reservationTypesCount(spark: SparkSession, hotelsReservation: DataFrame): DataFrame = {
    import spark.implicits._
